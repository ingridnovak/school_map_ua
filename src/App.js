...
function App() {
    ...
    const verifyAndLoadUser = async () => {
      ...
      if (isLoggedIn && token && hasUser) {
        try {
          ...
          // Fetch user's donation status
          try {
            const donationStatus = await api.getDonationStatus();
            if (donationStatus.data?.donation?.amount) {
              setUserDonationAmount(donationStatus.data.donation.amount);
            }
          } catch (error) {
  ...
  return (
    <div className="App">
      <a href="tel:+380500000000" className="contact-info" title="Зателефонувати для допомоги">
        <svg
          width="16"
          height="16"
          viewBox="0 0 24 24"
        ...
        <span>Виникли питання? +380 50 000 00 00</span>
      </a>
        ...
        {currentUser ? (
          <>
            ...
            {userDonationAmount > 0 && (
              <div
                className="user-donation-badge"
                title="Ваш благодійний внесок"
              >
                <svg
                  width="16"
                  height="16"
                ...
                <span>Добровільний внесок : {userDonationAmount} грн</span>
              </div>
            )}
            {hasCertificate && (
              <button
