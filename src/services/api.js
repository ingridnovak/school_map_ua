...
const handleResponse = async (response) => {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      throw new Error(
        errorData.error?.message || `Помилка сервера: ${response.status}`
      );
    } catch (parseError) {
      // If JSON parsing fails, the server returned non-JSON (like HTML error page)
      if (parseError instanceof SyntaxError) {
        throw new Error(
          `Сервер недоступний або повернув некоректну відповідь (${response.status})`
        );
      }
      throw parseError;
    }
...
export const api = {
  ...
  register: async (userData) => {
    const response = await fetch(`${API_BASE_URL}/auth/register`, {
      ...
      body: JSON.stringify({
        ...
        studentClass:
          userData.userType === "student" || userData.userType === "guest"
  ...
  getAllApprovedPins: async (limit = 1000, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/adventures/pins/all?limit=${limit}&offset=${offset}`
    );
    return handleResponse(response);
  },
  ...
  getPinsByRegion: async (
    ...
    const response = await fetch(
      `${API_BASE_URL}/adventures/pins/${encodeURIComponent(
        regionId
  ...
  deleteTestResult: async (regionId) => {
    const response = await fetch(
      `${API_BASE_URL}/tests/results/${encodeURIComponent(regionId)}`,
      {
        method: "DELETE",
        headers: getAuthHeaders(),
      }
  ...
  getRegionDonations: async (regionId) => {
    const response = await fetch(
      `${API_BASE_URL}/donations/region/${encodeURIComponent(regionId)}`
    );
    return handleResponse(response);
  },
  ...
  getClassDonationsTotal: async (className) => {
    const response = await fetch(
      `${API_BASE_URL}/donations/class/${encodeURIComponent(className)}/total`
    );
    return handleResponse(response);
  },
  ...
  getCertificateInfo: async () => {
    const response = await fetch(
      `${API_BASE_URL}/certificates/my-certificate`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  downloadCertificate: async () => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        error.error?.message || "Не вдалося згенерувати сертифікат"
      );
    }
  ...
  getPendingPins: async (limit = 200, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/pins/pending?limit=${limit}&offset=${offset}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  getPinsByClass: async (className, limit = 100, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/pins/class/${encodeURIComponent(
        className
  ...
  deleteUserPermanent: async (userId) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/users/${userId}/permanent`,
      {
        method: "DELETE",
        headers: getAuthHeaders(),
      }
  ...
  getStudentsByClass: async (className) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/users/class/${encodeURIComponent(className)}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  getDonationsByClass: async (className, limit = 50, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/donations/class/${encodeURIComponent(
        className
      )}?limit=${limit}&offset=${offset}`,
      {
        headers: getAuthHeaders(),
  ...
  getTestsByClass: async (className, limit = 50, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/tests/class/${encodeURIComponent(
        className
      )}?limit=${limit}&offset=${offset}`,
      {
        headers: getAuthHeaders(),
  ...
  getGuestDonations: async (limit = 50, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/donations/guests?limit=${limit}&offset=${offset}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  getUserDonation: async (userId) => {
    const response = await fetch(
      `${API_BASE_URL}/admin/donations/user/${userId}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  setUserDonation: async (userId, amount, notes = "") => {
    const response = await fetch(
      `${API_BASE_URL}/admin/donations/user/${userId}/set`,
      {
        method: "POST",
        headers: getAuthHeaders(),
  ...
  getAllTestResults: async (limit = 50, offset = 0) => {
    const response = await fetch(
      `${API_BASE_URL}/superadmin/tests?limit=${limit}&offset=${offset}`,
      {
        headers: getAuthHeaders(),
      }
    );
